...
void BillBoard::OnRender(glm::mat4 ModelMatrix)
{
	...
	if (this->bIsShearable) LifeLevel = (sin((ptrApp->GetApplicationTime() + fDeltaShear) * 5) - 0.2f) * 0.05;

	glUniform1f(uiBillboardStatusId, LifeLevel);
	glUniform1f(uiBillboardColorMod, 0.35);
