...
int Application::Execute(double dElapsedTime)
{
	...
	for (vector<Entity*>::iterator entity = this->listOfEntities.begin(); entity != this->listOfEntities.end();) {
		if (*entity) {
			if ((*entity)->getTranslate().y < -1 && (*entity) != ptrcMainCamera && (*entity) != ptrlMainLight && (*entity)->bDeletable) {
	...
	for (auto light : listOfLight) {
		light->OnLoop(dElapsedTime);
	}
...
void Application::AddLight(Light *l)
{
	listOfLight.push_back(l);
}
...
void Application::StartWindow() {
	...
	ptrWindow = glfwCreateWindow(iWindowWidth, iWindowHeight, "Trab OAC - x FPS", NULL, NULL);
	if (ptrWindow == NULL) {
...
void Application::InitApplication() {
	...
	int shadowMapSize = 1024 * 2;			/* Tamanho do mapa de sombras */
	...
	this->uiBillboardColorMod = glGetUniformLocation(this->uiBillboardProgramId, "colorMod");
	...
	m4DepthProjection = glm::ortho<float>(-64, 64, -64, 64, -64, 20);

	for (auto light : this->listOfLight) {
		light->Initialize();
	}

	for (auto light : this->listOfBillboards) {
		light->Initialize();
	}
	...
	do {
		...
		if (ptraCameraTarget) {
			if (this->ptrcMainCamera) {
				...
				// + vec3(0, 0.65, 0)
				vec3 sum = (this->ptraCameraTarget->getTranslate() + vec3(0, 1, -10));
			...
			if (this->ptrcMainCamera) {
				ptrController->computeMatricesFromInputs(elapsedTime, this->ptrv3LookAtPos, new vec3(this->ptrcMainCamera->getTranslate()));
			} else 
