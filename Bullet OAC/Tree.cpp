...
void Tree::Initialize()
{
	...
	tree->setTranslate(glm::vec3(0,-0.2,0));
	tree->setRotate(vec3(0, rand() % 360, 0));
	tree->setShearable(true);
	this->listOfEntities.push_back(tree);

	ParticleSystem* ps = new ParticleSystem("textures/folha.png", 50, vec3(0, 0, 0), vec3(0.0f, 0.0f, 0.0f));
	...
	ps->SetBirthFunction([](double dDeltaTime, ParticleSystem::Particle* p, ParticleSystem* ps) {
		if (p->fLife > 0) return;
		p->fLife = 10.0f;
		float radius = Utilities::map(rand() % 1000, 0, 1000, 0.5, 1.5);
		float dir1 = Utilities::map(rand() % 1000, 0, 1000, 0, M_PI);
		float dir2 = Utilities::map(rand() % 1000, 0, 1000, 0, M_PI);
